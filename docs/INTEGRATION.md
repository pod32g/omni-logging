...
Canonical event IDs are generated by the server. A top-level `id` supplied by
a producer is ignored so ingest clients cannot replace an existing event; use a
separate attribute such as `external_id` when a producer identifier is useful.
Timestamps more than 24 hours in the future are rejected.
